...
const carpetLayer = document.getElementById('carpetLayer');
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    @media(max-width:700px){
      .game-piece.normal{width:14.4%}.game-piece.khan{width:16.2%}
...
function applySceneLayout() {
  ...
  const px = v => `${(Number(v) || 0) * 100}%`;

  shell.style.setProperty('--scene-carpet-x', px(scene.carpetCenterX ?? .5));
  shell.style.setProperty('--scene-carpet-y', px(scene.carpetCenterY ?? .408));
  shell.style.setProperty('--scene-carpet-w', px(scene.carpetWidth ?? .82));
  shell.style.setProperty('--scene-carpet-h', px(scene.carpetHeight ?? .29));

  if (backgroundLayer && scene.backgroundImage) {
    backgroundLayer.style.backgroundImage = `url("${scene.backgroundImage.replace('./','')}")`;
  }
  if (carpetLayer && scene.carpetImage) {
    carpetLayer.style.backgroundImage = `url("${scene.carpetImage.replace('./','')}")`;
  }
}

function getCarpetGeometry(hostRect = host.getBoundingClientRect()) {
  const rect = carpetLayer?.getBoundingClientRect();
  if (rect && rect.width > 10 && rect.height > 10) {
    return {
      cx: rect.left - hostRect.left + rect.width / 2,
      cy: rect.top - hostRect.top + rect.height / 2,
      // The source WEBP has a few transparent pixels around the felt edge.
      rx: rect.width * .485,
      ry: rect.height * .485,
      width: rect.width,
      height: rect.height,
  ...
  const scene = CONFIG.scene || {};
  return {
    cx: hostRect.width * (scene.carpetCenterX ?? .5),
    cy: hostRect.height * (scene.carpetCenterY ?? .408),
    rx: hostRect.width * (scene.carpetWidth ?? .82) * .485,
    ry: hostRect.height * (scene.carpetHeight ?? .29) * .485,
    width: hostRect.width * (scene.carpetWidth ?? .82),
    height: hostRect.height * (scene.carpetHeight ?? .29),
  };
}

function outsideCarpet(x, y, carpet, threshold = 1) {
  const nx = (x - carpet.cx) / Math.max(1, carpet.rx);
  const ny = (y - carpet.cy) / Math.max(1, carpet.ry);
...
function buildPieces() {
  ...
  const scene = CONFIG.scene || {};
  const pileX = (scene.pileCenterX ?? .50) * 100;
  const pileY = (scene.pileCenterY ?? .455) * 100;
  state.pieces.push({
    id: 'KHAN',
    type: 'khan',
    src: khanFiles[Math.floor(Math.random() * khanFiles.length)],
    x: pileX - 3 + Math.random() * 6,
    y: pileY - 2 + Math.random() * 4,
...
function makeRandomScatter(count) {
  const out = [];
  const scene = CONFIG.scene || {};
  const cx = (scene.pileCenterX ?? .50) * 100;
  const cy = (scene.pileCenterY ?? .455) * 100;
  const rx = (scene.pileRadiusX ?? .335) * 100;
  const ry = (scene.pileRadiusY ?? .118) * 100;
  let minDist = 11.3;
  for (let i = 0; i < count; i++) {
    ...
    for (let attempt = 0; attempt < 300; attempt++) {
      ...
      if (p.x < 8 || p.x > 92 || p.y < 27 || p.y > 64) continue;
...
async function strikeTarget(source, target) {
  ...
  const hit = await animateChukoStrike2072(source, target);
  if (!hit) {
    state.phase = 'idle';
    syncSelectorLock();
    refreshPieceVisuals();
...
async function animateChukoStrike2072(source, target) {
  ...
  // v20.72 collision constants: heavy striker vs ordinary chuko, restitution 0.82.
  const strikerMass = 1.7;
  const targetMass = 1.0;
  const restitution = 0.82;
  const incomingSpeed = 18.5;
  const normalVelocity = -incomingSpeed;
  let impulse = -(1 + restitution) * normalVelocity;
  impulse /= (1 / strikerMass) + (1 / targetMass);
  const targetSpeed = impulse / targetMass;

  // The transparent WebP canvas is larger than the visible bone. A smaller
  // geometric radius lets the visible shapes actually touch at impact.
  const contactGap = Math.max(18, (sr.width + tr.width) * .19);
  ...
  const sourceEndLeft = impactX - hostRect.left;
  const sourceEndTop = impactY - hostRect.top;

  const baseRot = source.rotation;
  const impactRot = normalizeDeg(baseRot + (ux >= 0 ? 24 : -24));

  // Compute a guaranteed one-shot exit beyond the visible carpet, based on
  // the same "single eject animation" principle used in 20.72.
  const targetLocalX = tx - hostRect.left;
  const targetLocalY = ty - hostRect.top;
  // The physical rim is now read from the actual independent carpet layer.
  // Moving/scaling the carpet automatically moves/scales the collision rim.
  const carpet = getCarpetGeometry(hostRect);

  // Small tangential variation, as in 20.72, while keeping the hit direction dominant.
  const tangentAmount = (Math.random() * .24) - .12;
  ...
  if (!Number.isFinite(edgeDistance) || edgeDistance < 24) {
    edgeDistance = Math.min(hostRect.width * .24, 135);
  }

  // Travel just beyond the real felt rim, not to an abstract screen ellipse.
  // Keep enough room that the chuko remains visible after it leaves the carpet.
  const requestedExtra = clamp(CONFIG.scene?.edgeOutsetPx ?? 28, 16, 42);
  const halfW = tr.width * .50;
  const halfH = tr.height * .50;
  const safeMargin = 7;

  const safeCandidates = [];
  if (dirX > .001) safeCandidates.push((hostRect.width - halfW - safeMargin - targetLocalX) / dirX);
  if (dirX < -.001) safeCandidates.push((halfW + safeMargin - targetLocalX) / dirX);
  if (dirY > .001) safeCandidates.push((hostRect.height - halfH - safeMargin - targetLocalY) / dirY);
  if (dirY < -.001) safeCandidates.push((halfH + safeMargin - targetLocalY) / dirY);
  const safeLimit = Math.min(...safeCandidates.filter(v => Number.isFinite(v) && v > 0));

  // The centre must cross the rim. If viewport clearance is tight, prefer a
  // small amount of clipping over stopping inside the carpet.
  const minOutside = edgeDistance + 7;
  const wanted = edgeDistance + requestedExtra;
  const travel = Number.isFinite(safeLimit) && safeLimit > minOutside
    ? Math.min(wanted, safeLimit)
    : wanted;

  let endX = targetLocalX + dirX * travel;
  let endY = targetLocalY + dirY * travel;
  endX = clamp(endX, halfW + safeMargin, hostRect.width - halfW - safeMargin);
  endY = clamp(endY, halfH + safeMargin, hostRect.height - halfH - safeMargin);

  // Numerical / viewport safety: the final centre is always outside the real rim.
  if (!outsideCarpet(endX, endY, carpet, 1.015)) {
    const nx = (endX - carpet.cx) / Math.max(1, carpet.rx);
    const ny = (endY - carpet.cy) / Math.max(1, carpet.ry);
    const n = Math.max(.001, Math.hypot(nx, ny));
    const k = 1.025 / n;
    endX = carpet.cx + (endX - carpet.cx) * k;
    endY = carpet.cy + (endY - carpet.cy) * k;
  }

  const pushDistance = clamp(targetSpeed * 2.55, 36, 54);
  const c1x = targetLocalX + dirX * pushDistance + (-dirY) * tangentAmount * 22;
  const c1y = targetLocalY + dirY * pushDistance + dirX * tangentAmount * 22;
  const c2x = endX - dirX * 48 + (-dirY) * tangentAmount * 10;
  const c2y = endY - dirY * 48 + dirX * tangentAmount * 10;
  const targetSpin = ((Math.random() * 2.8) - 1.4) * Math.PI;
  ...
  // Striker reaches the target fast. Impact happens at 84% of this flight.
  const sourceDuration = 360;
  const impactAt = 0.84;
  const sourceAnim = sourceEl.animate([
    ...
    {
      offset: .15,
      left: `${sourceStartLeft - ux * 7}px`,
      top: `${sourceStartTop - uy * 7}px`,
      transform: `translate(-50%,-50%) rotate(${baseRot - (ux >= 0 ? 4 : -4)}deg) scale(1.015)`
    },
    {
      offset: impactAt,
      left: `${sourceEndLeft}px`,
      top: `${sourceEndTop}px`,
    ...
    easing: 'cubic-bezier(.18,.74,.16,1)',
    fill: 'forwards'
  });

  const impactDelay = Math.round(sourceDuration * impactAt);

  // Start the target on the exact impact frame — there is no post-hit wait.
  const targetPromise = new Promise(resolve => {
    setTimeout(() => {
      ...
      const startLeft = targetLocalX;
      const startTop = targetLocalY;
      const duration = 610 + Math.random() * 150;

      const startTime = performance.now();
      function tick(now) {
        const raw = Math.min(1, (now - startTime) / duration);
        const t = 1 - Math.pow(1 - raw, 3); // easeOutCubic from v20.72
        const mt = 1 - t;
  ...
  source.x = (sourceEndLeft / hostRect.width) * 100;
  source.y = (sourceEndTop / hostRect.height) * 100;
...
window.UPAY2D = {
  setSceneLayout(partial = {}) {
    if (!partial || typeof partial !== 'object') return { ...CONFIG.scene };
