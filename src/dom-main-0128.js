...
async function animateSourceToTarget(source, target, isKhan = false) {
  ...
  const scx = sr.left + sr.width / 2, scy = sr.top + sr.height / 2, tcx = tr.left + tr.width / 2, tcy = tr.top + tr.height / 2;
  const dx = tcx - scx, dy = tcy - scy, d = Math.max(1, Math.hypot(dx, dy)), ux = dx / d, uy = dy / d;
  const overlap = isKhan ? tr.width * .20 : tr.width * .18;
  const impactCx = tcx - ux * overlap, impactCy = tcy - uy * overlap;
  const clone = sourceEl.cloneNode(true);
  clone.classList.remove('selected', 'source-ready', 'valid-target', 'invalid-target', 'aim-candidate', 'pose-guide-match', 'pose-guide-dim');
  Object.assign(clone.style, { position: 'fixed', left: `${sr.left}px`, top: `${sr.top}px`, width: `${sr.width}px`, height: `${sr.height}px`, margin: '0', transform: 'none', zIndex: '10080', pointerEvents: 'none', opacity: '1' });
  document.body.appendChild(clone);
  sourceEl.style.visibility = 'hidden';
  const endLeft = impactCx - sr.width / 2, endTop = impactCy - sr.height / 2;
  const backLeft = sr.left - ux * 12, backTop = sr.top - uy * 12;
  const anim = clone.animate([
    { offset: 0, left: `${sr.left}px`, top: `${sr.top}px`, transform: 'rotate(0deg) scale(1)' },
    { offset: .18, left: `${backLeft}px`, top: `${backTop}px`, transform: 'rotate(-8deg) scale(1.02)' },
    { offset: .78, left: `${endLeft}px`, top: `${endTop - 9}px`, transform: `rotate(${ux >= 0 ? 148 : -148}deg) scale(1.06)` },
    { offset: 1, left: `${endLeft}px`, top: `${endTop}px`, transform: `rotate(${ux >= 0 ? 190 : -190}deg) scale(1)` },
  ], { duration: 720, easing: 'cubic-bezier(.20,.68,.18,1)', fill: 'forwards' });
  await anim.finished.catch(() => {});
  createImpactBurst(tcx, tcy);
  clone.remove();

  const br = shell.getBoundingClientRect();
  source.x = ((impactCx - br.left) / br.width) * 100;
  source.y = ((impactCy - br.top) / br.height) * 100;
  source.rotation = normalizeDeg(source.rotation + (ux >= 0 ? 14 : -14));
  ...
  return { ux, uy, impactCx, impactCy };
}

async function ejectTargetToCarpetEdge(target, ux, uy) {
  const el = target?.el;
  if (!el) return;
  const br = shell.getBoundingClientRect(), r = el.getBoundingClientRect();
  const cx = r.left + r.width / 2, cy = r.top + r.height / 2;
  const localX = cx - br.left, localY = cy - br.top;
  const carpet = { cx: br.width * .50, cy: br.height * .455, rx: br.width * .435, ry: br.height * .175 };

  const px = localX - carpet.cx, py = localY - carpet.cy;
  ...
  if (!Number.isFinite(t) || t < 40) t = Math.min(br.width * .25, 145);
  t += Math.min(18, r.width * .16);

  let landX = localX + ux * t;
  let landY = localY + uy * t;
  const halfW = r.width * .52, halfH = r.height * .52;
  landX = clamp(landX, halfW + br.width * .025, br.width - halfW - br.width * .025);
  landY = clamp(landY, br.height * .285 + halfH, br.height * .625 - halfH);

  const dx = landX - localX, dy = landY - localY;
  const base = el.style.transform;
  const spin = (ux >= 0 ? 1 : -1) * (115 + Math.random() * 55);
  const anim = el.animate([
    { offset: 0, transform: base },
    { offset: .56, transform: `${base} translate(${dx * .62}px,${dy * .62 - 12}px) rotate(${spin * .56}deg) scale(1.03)` },
    { offset: .88, transform: `${base} translate(${dx}px,${dy - 3}px) rotate(${spin}deg) scale(.98)` },
    { offset: 1, transform: `${base} translate(${dx}px,${dy}px) rotate(${spin * 1.04}deg) scale(.98)` },
  ], { duration: 690, easing: 'cubic-bezier(.18,.62,.22,1)', fill: 'forwards' });
  await anim.finished.catch(() => {});
...
function animateMiss(source, missDx, missDy, onDone) {
  ...
  ], { duration: 760, easing: 'cubic-bezier(.18,.7,.2,1)', fill: 'forwards' }).onfinish = () => { clone.remove(); el.style.visibility = ''; onDone?.(); };
}
